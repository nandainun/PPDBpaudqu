                ...
                <div class="modal fade" id="tambahdata" tabindex="-1" role="dialog" aria-labelledby="modelTitleId" aria-hidden="true">
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            ...
                            <form id="form-bayar">
                                <div class="modal-body">
                                    <input type="hidden" value="<?= $siswa['id_daftar'] ?>" name="id_daftar">
                                    <div class="form-group">
                                        <label for="jumlah">Jumlah Pembayaran Rp.</label>
                                        <input type="text" class="form-control" name="jumlah_bayar" id="jumlah_bayar" aria-describedby="helpjumlah" required>
                                    </div>
                                    <div class="form-group">
                                        <label for="tgl">Tanggal Pembayaran</label>
                                        <input type="text" class="form-control datepicker" name="tgl_bayar" id="tgl_bayar" required>
                                    </div>
                                    <div class="form-group">
                                        <label for="bukti">Bukti Pembayaran</label>
                                        <input type="file" class="form-control-file" name="bukti" id="bukti" accept="image/*" aria-describedby="fileHelpId" required>
                                        <small id="fileHelpId" class="form-text text-muted">Upload file JPG/JPEG/PNG</small>
                                    </div>
                                </div>
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                    ...
                    <table class="table table-striped table-sm" id="tablebayar" style="font-size: 12px">
                        ...
                        <tbody>
                            ...
                            while ($bayar = mysqli_fetch_array($query)) {
                                $user = fetch($koneksi, 'bayar', ['id_bayar' => $bayar['id_daftar']]);
                                $no++;
                            ?>
                                <tr>
                                    <td><?= $no; ?></td>
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data == 'ok') {
                    ...
                    iziToast.error({
                        title: 'Maaf!',
                        message: 'Refresh kembali website atau ulangi tambah pembayaran!',
                        position: 'topRight'
                    });
                }
